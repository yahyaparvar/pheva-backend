...
aiRouter.post("/emails/summary", async (req, res) => {
  ...
  try {
    ...
    const summaries = gptResponse.choices[0].message.content;
    res.json({ emails: JSON.parse(summaries || "{}").emails });
  } catch (error) {
...
aiRouter.post("/emailsDetails/summary", async (req, res) => {
  ...
  const prompt = `
  Here is user's Email text content extracted
  ${userPrompt}
  send me a summary of this email
  keep it under 5 lines
  single, continuous string and 5 lines. without any numbering, special formatting, line breaks, or extra spaces, ensuring the text is suitable for direct rendering in a frontend application without any errors.`;
...
aiRouter.post("/emailsDetails/negativeAnswer", async (req, res) => {
  const { prompt: userPrompt, name } = req.body;
  const prompt = `
  This is the user's email content extracted from HTML and converted to plain text:
  "${userPrompt}"
  
  Please write a negative and polite response to the email, especially if it contains a question or request. Examples of negative responses include phrases like "Unfortunately, we cannot proceed" or "I'm sorry, but I cannot do that."
  Keep it under 7 lines and more than 4 lines.
  without any numbering, special formatting, line breaks, or extra spaces, ensuring the text is suitable for direct rendering in a frontend application without any errors.
  `;
...
aiRouter.post("/emailsDetails/positiveAnswer", async (req, res) => {
  ...
  const prompt = `
    ...
    Please write a positive and affirmative response to the email, especially if it contains a question or request. Examples of positive responses include phrases like "We are happy to move forward" or "I am happy to do that."
    Keep it under 7 lines and more than 4 lines.
    without any numbering, special formatting, line breaks, or extra spaces, ensuring the text is suitable for direct rendering in a frontend application without any errors.
    `;
